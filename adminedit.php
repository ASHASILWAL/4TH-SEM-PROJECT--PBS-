...
ob_start(); 
  require_once 'db.php';
   ...
   $qu = "UPDATE admindetails SET name='$fname', username= '$username',email= '$email' 
   WHERE contact_id ='$profileid'";
    $run_query =@mysqli_query($dbcon,$qu);
	if($run_query){
		
    header("Location:adminprofile.php?update=profileupated"); 
   }else  {
   ...
   $profileid = $_GET['editaProfile']; // GETTING ID FROM URL
   $query = "SELECT * FROM admindetails WHERE contact_id ='$profileid' ";
...
<?php  include_once 'amenu.php';?><br><br>
 ...
 <label>USERNAME</label> <input type="text" name ="uname" value="<?php echo $drow['usernames'];?>"><br>
  ...
  <style>
	body{
	background-image: url("adminedit.png");
